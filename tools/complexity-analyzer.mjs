...
const loadTypeScript = async (sourcePath) => {
  const modulePath = resolveTypeScript(sourcePath)
...
const analyzeFunction = (node, sourceFile, ts) => {
  ...
  const visit = (current) => {
    ...
    const decision = isDecisionNode(current, ts)
...
export const analyzeSource = async (sourcePath) => {
  ...
  const visit = (node) => {
    if (isFunction(node, ts)) {
      ...
      metric.tsxCompositionException =
        extension === '.tsx' &&
        /[A-Z]/.test(metric.name[0] ?? '') &&
